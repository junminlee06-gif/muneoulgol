...
mark.id = 'ch1Mark';
mark.textContent = 'expansion v2026-06-11b';
...
function handleP(place) {
  ...
  if (P.stage === 'end') return place === 'office' ? openFinalCh1Report() : toast('관아 임시처소에서 마무리하자.');
}
function day1(place) {
  ...
  const data = {
    chief: ['을파의 장부', '촌장집 마루는 볕을 잘 받는데도 장부 위만 눅눅하다. 을파는 장부 한 칸을 손바닥으로 가린다.\n\n그 칸에는 산제에 쓴 쌀과 술의 수량이 있어야 했다. 그러나 먹이 번진 빈칸만 남아 있다.', [
      opt('범의 흔적으로 본다.', { b: [['beast', 1]], o: ['촌장 진술: 범 가능성'] }, '을파는 기다렸다는 듯 고개를 끄덕인다. “그렇게 적으면 마을이 덜 흔들릴 겁니다.”'),
    ...
    well: ['우물가', '우물가에는 아이들이 남긴 발자국이 겹겹이 말라 있다. 물은 조용한데, 우물 안쪽에서 산길 물소리가 난다.', [
      opt('도리의 이름을 확인한다.', { b: [['human', 1]], o: ['이름 확인: 도리'], c: ['도리의 이름'] }, '월선은 이름을 듣자마자 고개를 든다. “도리요. 아직 도리라고 불러야 합니다.”'),
      opt('산제 노래를 묻는다.', { b: [['ritual', 1]], c: ['산제 노래'] }, '한 아이가 노래의 첫 구절을 부르다 말고 입을 다문다.'),
      opt('주민 침묵을 본다.', { b: [['conceal', 1]], n: ['사람들이 서로 눈치를 본다'] }, '모두가 말하지 않기로 한 문장 앞에서 입술을 닫고 있다.')]],
    shaman: ['막례의 말', '무당집 처마 밑 금줄은 오래되어 누렇게 말랐다. 그런데 매듭 하나만 새 짚이다.\n\n“산은 기록을 못 해. 대신 사람이 산 대신 거짓말을 하지.”', [
      opt('산제 의미를 묻는다.', { b: [['ritual', 1]], c: ['검은등 별칭'] }, '“검은등은 이름이 아니야. 등을 보였다는 뜻이지.”'),
      opt('사람의 개입을 묻는다.', { b: [['human', 1]], n: ['사람이 산 이름을 빌릴 때가 있다'] }, '“산이 아이를 부를 때도 있고, 사람이 산 목소리를 흉내 낼 때도 있어.”'),
      opt('공식 감시 대상으로 본다.', { b: [['conceal', 1]], o: ['무당 감시 필요성'] }, '“관아 문서에 내 이름을 올려. 대신 아이 이름도 같이 올려.”')]],
    mill: ['방앗간', '방앗간은 낮인데도 서늘하다. 물레는 멈췄고 물길도 막혔는데, 축에는 방금 젖은 듯한 물방울이 맺혀 있다.', [
      opt('풀 조각을 챙긴다.', { b: [['human', 1]], c: ['짚신 풀 조각'] }, '풀 조각은 붙잡힌 쪽이 버틴 흔적에 가깝다.'),
      opt('물기를 확인한다.', { b: [['ritual', 1]], c: ['젖은 물레 축'] }, '손끝에 닿는 순간 오래된 제삿술 냄새가 난다.'),
      opt('검은 털을 줍는다.', { b: [['beast', 1]], c: ['검은 털'] }, '검은 털은 범의 것이라 적기에는 너무 젖어 있다.')]]
  }[place];
...
function openFirstReportPanel() {
  ...
  intro.textContent = '첫날 낮 조사 결과를 관아 문장으로 정리합니다. 제출 후에는 1일차 밤 조사로 이어집니다.';
...
function finishNight1(afterText) {
  showP('1일차 밤 기록', afterText, [{ text: '2일차 아침으로 넘긴다', run: beginDay2 }]);
}
function night1(place) {
  if (!['well', 'shaman', 'mill'].includes(place)) return toast('첫날 밤에는 우물, 무당집, 방앗간 중 한 곳만 확인하자.');
  const data = {
    well: { title: '첫날 밤의 우물', body: '우물가에는 아무도 없다. 물 위에는 우물가에 없는 장승 그림자가 비친다.', e: { b: [['ritual', 1]], c: ['우물의 이름 반응'], n: ['도리 이름에 우물이 반응했다'] }, response: '도리의 이름을 적자 우물 안쪽에서 같은 이름이 아주 작게 되돌아온다.', after: '첫날 밤 기록: 우물은 범의 발자국도 사람의 손자국도 내놓지 않았다. 대신 이름에 반응했다.' },
    shaman: { title: '첫날 밤의 무당집', body: '무당집에는 등불이 없다. 금줄의 짚 끝마다 희미한 빛이 걸려 있다.\n\n막례는 “보고서에 적은 말이 밤을 부른다”고 말한다.', e: { b: [['ritual', 1]], c: ['밤의 금줄'], n: ['막례는 창귀 가능성을 에둘러 말했다'] }, response: '막례는 끝내 검은등이라는 이름을 입 밖에 내지 않는다. 대신 문턱 아래의 짚 조각을 가리킨다.', after: '첫날 밤 기록: 막례는 산이 아이를 데려갔다고 말하지 않았다. 다만 사람이 산의 이름을 빌릴 수 있다고 했다.' },
    mill: { title: '첫날 밤의 방앗간', body: '방앗간의 물레는 멈춰 있다. 하지만 멈춘 물레에서 물이 도는 소리만 빠져나온다.', e: { b: [['human', 1], ['ritual', 1]], c: ['밤 방앗간의 물소리'], n: ['방앗간에서 아이 목소리 같은 소리를 들었다'] }, response: '물레 소리 사이로 아주 낮은 목소리가 섞인다. 아이 목소리 같기도 하고, 어른이 아이 흉내를 내는 소리 같기도 하다.', after: '첫날 밤 기록: 방앗간은 산의 흔적과 사람의 흔적을 동시에 남겼다. 어느 한쪽만 적으면 나머지 하나가 거짓이 된다.' }
  }[place];
...
function day2(place) {
  ...
  const data = {
    ...
    well: ['2일차 우물가', '월선은 우물가에 앉아 도리의 이름을 반복하고 있다.', [
    ...
    mill: ['2일차 방앗간', '방앗간의 물레는 완전히 말랐다. 말랐기 때문에 더 수상하다.', [
      opt('검은 털을 보관한다.', { b: [['beast', 1]], c: ['젖은 검은 털'] }, '검은 털은 아침이 되자 조금 가벼워졌다.'),
...
function dusk2() {
  P.stage = 'dusk';
  showP('2일차 해질녘', '둘째 날 낮 조사가 끝났다. 이제 누구의 말을 더 무겁게 둘지 정해야 한다.\n\n이 선택 뒤에 두 번째 밤이 온다. 마지막 보고서는 아직 아니다.', [
    { text: '을파의 설명을 따른다', e: { b: [['beast', 1]] }, response: '마을은 잠시 안정된다. 하지만 안정된 마을일수록 누군가의 울음소리는 더 멀리 밀려난다.', run: startNight2 },
    { text: '월선의 증언을 믿는다', e: { b: [['human', 1]] }, response: '월선은 도리의 이름을 다시 붙잡는다.', run: startNight2 },
    { text: '막례의 경고를 듣는다', e: { b: [['ritual', 1]] }, response: '막례가 금줄을 한 겹 더 묶는다.', run: startNight2 },
...
function startNight2() {
  ...
  P.max = 1;
  updateShade();
  showP('2일차 밤', '두 번째 밤이다. 이번 밤을 지나야 1장의 마지막 보고서를 쓸 수 있다.\n\n촌장집, 우물, 무당집, 방앗간 중 한 곳을 마지막으로 확인하자.', [{ text: '두 번째 밤 조사를 시작한다', run: closeDialog }]);
}
function night2(place) {
  if (!['chief', 'well', 'shaman', 'mill'].includes(place)) return toast('두 번째 밤에는 촌장집, 우물, 무당집, 방앗간 중 한 곳을 보자.');
  const data = {
    chief: ['두 번째 밤: 촌장집', '헛간 쪽에서 젖은 짚 냄새가 난다. 낮에는 없던 냄새다.', { b: [['conceal', 1]], c: ['젖은 짚단'] }],
    well: ['두 번째 밤: 우물', '물은 같은 이름을 여러 사람의 목소리로 나누어 돌려준다.', { b: [['ritual', 1]], c: ['이름 반응'] }],
    shaman: ['두 번째 밤: 무당집', '금줄이 한 겹 더 매인다. 오래된 의례는 기도보다 수선에 가깝다.', { b: [['ritual', 1]], c: ['제문 조각'] }],
    mill: ['두 번째 밤: 방앗간', '물이 없는 곳에서 물소리가 난다. 물이 아니라 길이 흐른다는 뜻일지도 모른다.', { b: [['unknown', 1]], n: ['물소리 속에 여러 이름이 섞였다'] }]
  }[place];
  showP(data[0], data[1], [{ text: '새벽까지 기록한다', e: data[2], response: '두 번째 밤 기록이 끝났다. 이제서야 1장의 마지막 보고서를 쓸 수 있다.', run: end1 }]);
}
function end1() {
  P.stage = 'end';
  updateShade();
  showP('1장 새벽 정리', '도리는 돌아오지 않았다.\n\n하지만 이제 그는 단순한 실종자가 아니다. 보고서에 적힌 이름, 우물에 되돌아온 이름, 누군가 숨기려 한 이름이 되었다.\n\n관아 임시처소에서 1장의 마지막 보고서를 작성하자.', [{ text: '관아에서 1장 마지막 보고서를 작성한다', run: openFinalCh1Report }]);
}
function openFinalCh1Report() {
  P.done = true;
  closeDialog();
  updateShade();
...
function ch2Day1(place) {
  const data = {
    valley: ['물소리 골짜기', '골짜기에는 흐르는 물이 없다. 그런데 귀를 낮추면 돌 밑에서 물이 흐른다. 발자국 몇 개가 같은 자리를 빙빙 돈다.', [
      opt('소리가 돌아오는 지점을 표시한다', { b: [['path', 1], ['unknown', 1]], c: ['되돌아오는 물소리'] }, '표시한 돌은 세 번 확인해도 제자리가 아니다.'),
      opt('발자국의 순서를 본다', { b: [['human', 1]], c: ['반복된 발자국'] }, '가장 작은 발자국 위에 더 큰 발자국이 겹쳐 있다.')]],
    lumber: ['벌목터', '벌목터의 나무들은 베인 것이 아니라 멈춘 것처럼 서 있다.', [
      opt('사량의 덫을 확인한다', { b: [['human', 1]], c: ['사량의 빈 덫'] }, '덫은 짐승을 잡기 위한 모양이지만, 끈 높이가 아이 허리쯤에 걸려 있다.'),
      opt('벌목 중단 이유를 묻는다', { b: [['conceal', 1]], n: ['벌목터 사람들은 같은 날 일을 멈췄다'] }, '사량은 “나무가 울어서”라고 말한다.')]],
    shrine: ['무너진 산제단', '산제단은 무너져 있지만, 무너진 순서가 이상하다. 누군가 폐허를 고치려다 중간에 그만둔 흔적이다.', [
    ...
    well: ['2장 우물가', '우물가의 물소리와 골짜기의 물소리가 같은 박자로 겹친다.', [
    ...
    mill: ['2장 방앗간', '방앗간의 물레 소리가 골짜기 쪽과 엇박으로 돈다.', [
      opt('물레 축과 골짜기 돌을 대조한다', { b: [['unknown', 1]], c: ['엇박의 물소리'] }, '두 소리는 서로를 흉내 낸다.'),
...
function startCh2Night1() {
  ...
  Q.max = 1;
  updateShade();
  showFor(Q, '2장 1일차 밤', '첫날 낮에 표시한 물소리는 해가 지자 마을 안쪽으로 번진다. 우물, 무당집, 방앗간, 골짜기 중 한 곳을 확인하자.', [{ text: '밤 조사를 시작한다', run: closeDialog }]);
}
function ch2Night1(place) {
  const data = {
    well: ['1일차 밤: 우물', '우물은 골짜기보다 먼저 어두워진다. 물 위에는 북쪽 길의 나뭇가지 그림자가 비친다.', { b: [['path', 1], ['ritual', 1]], c: ['마을로 번진 물소리'] }],
    shaman: ['1일차 밤: 무당집', '막례의 금줄 끝이 젖어 있다. 비가 오지 않았는데 짚에서는 물비린내가 난다.', { b: [['ritual', 1]], c: ['젖은 금줄'] }],
    mill: ['1일차 밤: 방앗간', '물레가 돌지 않는데도 방앗간 바닥에는 둥근 물자국이 생긴다.', { b: [['human', 1], ['unknown', 1]], c: ['둥근 물자국'] }],
    valley: ['1일차 밤: 골짜기', '밤의 골짜기는 같은 돌, 같은 나무, 같은 발자국을 세 번 보여준다.', { b: [['path', 1]], c: ['세 번 반복된 길'] }]
  }[place];
...
function beginCh2Day2() {
  ...
  Q.max = 4;
  updateShade();
  showFor(Q, '2장 2일차 낮', '아침이 되자 길은 다시 평범한 산길처럼 보인다. 하지만 전날 밤의 기록 때문에 볼 수 있는 것이 달라졌다.', [{ text: '2일차 낮 조사를 시작한다', run: closeDialog }]);
}
function ch2Day2(place) {
  const data = {
    forest: ['금지된 숲 입구', '금지된 숲 입구에는 금줄이 없다. 하지만 흙 위에는 최근 발자국이 있다.', [
      opt('발자국 방향을 본다', { b: [['human', 1]], c: ['숲 입구의 최근 발자국'] }, '발자국은 숲으로 들어간 것이 아니라, 숲에서 마을로 나온 쪽이 더 깊다.'),
      opt('금줄이 없는 이유를 생각한다', { b: [['ritual', 1]], n: ['금지된 숲에는 금줄이 없다'] }, '이곳은 이미 모두가 막혀 있다고 믿는 곳이다.')]],
    shrine: ['2일차 산제단', '어제 본 젖은 쌀알은 사라졌다. 대신 같은 자리에 마른 쌀겨가 둥글게 모여 있다.', [
      ...
      opt('최근 제물 흔적을 찾는다', { b: [['conceal', 1]], c: ['숨겨진 제물 흔적'] }, '누군가는 산제를 중단한 적이 없다.')]],
    lumber: ['2일차 벌목터', '벌목터의 덫은 치워져 있다. 하지만 덫이 있던 자리에는 아이 키 높이의 줄 자국이 남아 있다.', [
      opt('줄 자국을 잰다', { b: [['human', 1]], c: ['아이 키 높이의 줄 자국'] }, '짐승을 노린 덫이라기에는 너무 낮다.'),
      opt('사량의 거짓말을 확인한다', { b: [['conceal', 1]], n: ['사량은 덫을 치웠다'] }, '사량은 덫을 치운 적 없다고 말한다.')]],
    chief: ['2일차 촌장집', '을파는 북쪽 출입 명단이 없다고 말한다. 그런데 장부 끈에는 방금 풀었다 묶은 자국이 있다.', [
      ...
      opt('마루와 도리의 관계를 묻는다', { b: [['human', 1]], c: ['마루와 도리의 연결'] }, '두 이름은 같은 손글씨로 고쳐 적혀 있다.')]],
    ...
    mill: ['2일차 방앗간', '방앗간의 물레는 멈춰 있다. 하지만 그림자는 천천히 돈다.', [
      opt('그림자 방향을 기록한다', { b: [['unknown', 1]], c: ['반대로 도는 물레 그림자'] }, '그림자는 물레와 반대 방향으로 돈다.'),
      opt('벌목터 흙과 대조한다', { b: [['human', 1]], c: ['방앗간과 벌목터의 같은 흙'] }, '진흙 색이 같다.')]],
...
function ch2Report() {
  ...
  intro.textContent = '물소리 골짜기와 마을 내부 진술을 대조해 보고합니다. 제출 후 2일차 밤으로 이어집니다.';
  buildReport(groupsBox, report, [
  ...
  finishBtn.onclick = () => {
    ...
    Q.max = 1;
    updateShade();
    respondFor(Q, '2장 중간 보고서가 접수되었다. 그날 밤, 보고서에 적은 판단이 먼저 젖는다.', () => showFor(Q, '2장 2일차 밤', '물소리 골짜기, 산제단, 우물, 방앗간 중 한 곳에서 보고서의 반응을 확인하자.', [{ text: '마지막 밤 조사를 시작한다', run: closeDialog }]));
  };
  panel.style.display = 'block';
}
function ch2Night2(place) {
  const data = {
    valley: ['2일차 밤: 물소리 골짜기', '골짜기는 이번에는 길을 반복하지 않는다. 대신 당신이 낮에 적은 보고서 문장을 돌려준다.', { b: [['path', 1], ['unknown', 1]], c: ['보고서를 따라 읽는 골짜기'] }],
    shrine: ['2일차 밤: 무너진 산제단', '무너진 돌들이 낮과 다른 위치에 놓여 있다. 제단은 조금 더 완성된 모양에 가까워졌다.', { b: [['ritual', 1]], c: ['밤에 움직인 제단 돌'] }],
    well: ['2일차 밤: 우물', '우물물은 더 이상 이름 하나만 돌려주지 않는다. 도리, 마루, 아직 기록하지 않은 이름이 섞여 올라온다.', { b: [['human', 1], ['ritual', 1]], c: ['섞여 돌아온 이름들'] }],
    mill: ['2일차 밤: 방앗간', '방앗간 물레와 골짜기 물소리가 처음으로 같은 박자에 맞는다.', { b: [['human', 1], ['unknown', 1]], c: ['물레 그림자의 팔'] }]
  }[place];
  if (!data) return toast('2일차 밤에는 물소리 골짜기, 산제단, 우물, 방앗간 중 한 곳을 보자.'), true;
  showFor(Q, data[0], data[1], [{ text: '2장 밤의 결과를 기록한다', e: data[2], response: '2장 밤 기록이 끝났다. 이제 물소리 골짜기는 마을이 숨긴 이름들이 되돌아오는 통로가 되었다.', run: () => {
    Q.stage = 'end';
    updateShade();
    showFor(Q, '2장 종료', '새벽이 오자 북쪽 길은 다시 평범한 산길처럼 보인다. 관아 임시처소에서 2장 보고서를 정리하자.', [{ text: '관아에서 2장 보고서를 작성한다', run: openFinalCh2Report }]);
  } }]);
  return true;
}
...
setInterval(() => {
  ...
  side();
  maybeStartCh2();
  if (musicOn) playTrack(isNight() ? 'night' : 'day');
}, 900);
})();