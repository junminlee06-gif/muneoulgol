(() => {
  ...
  style.textContent = `
    .dialog{position:absolute!important;left:16px!important;right:16px!important;bottom:16px!important;top:auto!important;transform:none!important;max-height:340px!important;overflow:hidden!important;}
...
(() => {
  ...
  const locIds = { '촌장집': 'chief', '마을 우물': 'well', '무당집': 'shaman', '방앗간': 'mill', '관아 임시처소': 'office' };
  const patch = {
    started: false,
    finished: false,
    dialogOpen: false,
  ...
  function locationScene(title, body, options, loc) {
    showCustom(title, body, options.map(o => withRun(o, () => {
      if (consume(loc)) afterDayInvestigation();
    })));
  }
  function showD1Location(loc) {
    if (!['chief', 'well', 'shaman', 'mill'].includes(loc)) return toast('1일차 낮에는 촌장집, 우물, 무당집, 방앗간을 조사하자.');
    const scenes = {
      chief: ['을파의 장부', '을파는 장부를 닫지 않았다. 산제 비용 칸만 세 해째 비어 있다.', [
      ...
      shaman: ['막례의 무당집', '막례는 방울을 뒤집어 놓고 있었다. “산은 기록을 못 한다. 그래서 관아 사람이 온 게지.”', [
        opt('산제가 무엇을 막는 의례였는지 묻는다.', { bias: [['ritual', 1]], trust: [['makrye', 1]], clues: ['검은등이라는 산군 별칭'], unofficial: ['산제는 문을 닫는 의식에 가까웠다'] }),
    ...
    locationScene(scenes[loc][0], scenes[loc][1], scenes[loc][2], loc);
  }
  function showD1Dusk() {
  ...
  function showD1Night(loc) {
    ...
    const scenes = {
      ...
      shaman: ['막례의 밤', '막례는 잠들지 않았다. 금줄을 맨 손끝이 떨린다.', [
  ...
  function showD1Report() {
    ...
    showCustom('첫 보고서', '새벽이 되었다.\n\n보고서는 단순한 정리가 아니다. 무엇을 공식 진실로 만들 것인지 정하는 일이다.', [
      { text: '범의 습격 가능성', effect: { bias: [['beast', 2]], trust: [['office', 1], ['eulpa', 1], ['wolseon', -1]], official: ['1장 첫 보고: 범의 습격 가능성'], flags: [['hunterArrived', true]] }, run: () => beginDay2('beast') },
  ...
  function day2MorningText(kind) {
    if (kind === 'beast') return '범이라는 말은 마을을 겁주지만 동시에 안심시켰다. 장승 아래에는 낯선 사냥꾼이 서 있다.';
    if (kind === 'human') return '사람이 아이를 데려갔다는 문장은 범보다 깊게 마을을 물었다. 월선은 당신을 기다리고 있다.';
    if (kind === 'ritual') return '마을은 조용하다. 그러나 소리를 덮어 누르는 듯하다. 막례가 장승 뒤에서 기다린다.';
    if (kind === 'conceal') return '당신이 공터에 나서자 말소리가 끊겼다. 마을 전체가 용의자가 되었다.';
  ...
  function showD2Location(loc) {
    ...
    const scenes = {
      ...
      shaman: ['2일차 막례의 뒤뜰', '막례의 뒤뜰에는 말라붙은 풀과 젖은 돌이 함께 놓여 있다. “산은 제물을 달라 하지 않는다.”', [
  ...
  function finishToOriginal() {
    ...
    closeCustom();
    try {
      if (typeof window.openReport === 'function') window.openReport(1);
      else toast('원본 보고서 함수를 찾지 못했다. 새로고침 후 다시 시도해 주세요.');
    } catch (e) {
  ...
  window.addEventListener('keydown', e => {
    if (patch.dialogOpen) {
      ...
      if (actionKeys.has(e.key)) { e.preventDefault(); e.stopImmediatePropagation(); }
      return;
    }
    if (!actionKeys.has(e.key) || e.repeat || reportVisible() || originalDialogVisible() || !inCh1VillageContext()) return;
    const loc = currentLoc();
    if (!loc) return;
    if (['d1_day', 'd1_office', 'd1_night', 'd2_day', 'd2_night', 'd2_dawn'].includes(patch.stage)) {
      e.preventDefault();
